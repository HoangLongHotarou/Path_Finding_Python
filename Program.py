...
def test():
    ...
    def btn_add(open_img, type):
        btn_show = tkinter.Button(tab2, text=f"Open img_{type}", font=(
            "segoeuisymbol", 14), command=open_img)
        btn_show.pack()
    ...
    button = tkinter.Button(tab1, text="Start Maze on Pygame", font=(
        "segoeuisymbol", 14), command=action)
    button.pack()
